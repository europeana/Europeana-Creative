...
/**
 * ticket: <code>https://europeanadev.assembla.com/spaces/europeana-creative/tickets/2?comment=450587433#comment:450587433</code>
 * 
 ...
 * TYPE:IMAGE&rows=24 Amsterdam
 * museum:http://www.europeana.eu/portal/search.
 * html?query=*:*&qf=TYPE:IMAGE&
 * qf=PROVIDER:%22Digitale+Collectie%22&qf=DATA_PROVIDER
 * :%22Amsterdam+Museum%22&rows=12 Bildarchiv Foto
 * Marburg:http://www.europeana
 * .eu/portal/search.html?query=*:*&qf=TYPE:IMAGE
 * &qf=DATA_PROVIDER:%22Bildarchiv+Foto+Marburg%22&rows=12 National Library
 * of
 * Netherlands:http://www.europeana.eu/portal/search.html?query=*:*&qf=TYPE
 * :IMAGE
 * &qf=DATA_PROVIDER:%22National+Library+of+the+Netherlands+-+Koninklijke
 * +Bibliotheek%22&rows=12 Austrian National
 * Library:http://www.europeana.eu/
 * portal/search.html?query=*:*&qf=TYPE:IMAGE
 * &qf=DATA_PROVIDER:%22%C3%96sterreichische
 * +Nationalbibliothek+-+Austrian+National+Library%22&rows=12 Progetto Art
 * Past
 * :http://www.europeana.eu/portal/search.html?query=*%3A*&start=13&rows=
 * 48&qf
 * =TYPE%3AIMAGE&qf=DATA_PROVIDER%3A%22Progetto+ArtPast-+CulturaItalia%22
 **/
public class EuCreativeDemoDatasetTest extends
	...
	public void createAmsterdamMuseumSubset() throws MalformedURLException,
		...
		// Amsterdam museum:http://www.europeana.eu/portal/search.html?query=*:*&qf=TYPE:IMAGE&qf=PROVIDER:%22Digitale+Collectie%22&qf=DATA_PROVIDER:%22Amsterdam+Museum%22&rows=12
		...
		createSubset("digitalcollection", "Amsterdam_Museum", portalUrl, 1, 60413);
	}
	...
	protected void createSubset(String imageSetName, String collectionName, String portalUrl, int start, int expectedResults) throws MalformedURLException,
			UnsupportedEncodingException, IOException {
		
		DatasetDescriptor dataset = new DatasetDescriptor(imageSetName, collectionName);
		Api2QueryInterface query = getQueryBuilder().buildQuery(portalUrl);
		int objects = buildImageSet(dataset, query, start, -1, ThumbnailsAccessor.ERROR_POLICY_CONTINUE);
